function [comp_h, press_h, temp_h, pressbub_h, pressdew_h] = main_firoozabadi(h_target, h_ref, comp_ref, P_ref, T_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params)
% MAIN_FIROOZABADI - Non-isothermal Compositional Grading (Shukla-Firoozabadi Model)
% =========================================================================
% Implements Shukla-Firoozabadi (2000) thermal diffusion model:
%
%   Net Heat of Transport (Eq. 3.66 from Nikpoor 2014):
%     Q_i^net = (1/τ) * [-ΔU_i + (ΔU_m * v_i / V_m)]
%
%   Where:
%     ΔU_i = H_res_i - P*v_i  (internal energy departure)
%     ΔU_m = Σ(x_j * ΔU_j)    (mixture internal energy departure)
%     v_i  = partial molar volume [m³/mol]
%     V_m  = mixture molar volume [m³/mol]
%     τ    = empirical parameter (default = 4.0)
%
%   Main equation:
...
%   Cp_coeffs : Ideal gas Cp coefficients [n x 4] (kept for interface consistency)
%   H_ig_ref  : Reference ideal gas enthalpies [J/mol] (kept for interface consistency)
%   tau       : Empirical parameter (default = 4.0, literature range: 3-6)
...
% =========================================================================
% REFERENCES:
%   Shukla, K. & Firoozabadi, A. (1998). Ind. Eng. Chem. Res. 37, 3324-3335
%   Firoozabadi, A., Ghorayeb, K., & Shukla, K. (2000). AIChE J. 46, 892-900
...
% =========================================================================
    ...
    for step = 1:n_steps
        ...
        [comp_next, P_next, T_next] = firoozabadi_single_step(...
            h_next, h_current, comp_current, P_current, T_current, dTdh, ...
            Pc, Tc, acentric, BIP, M_gmol, tau, vt_type, eos_type, vt_opts);
...
function [comp_h, press_h, temp_h] = firoozabadi_single_step(h, h_ref, comp_ref, P, T, dTdh, Pc, Tc, acentric, BIP, M_gmol, tau, vt_type, eos_type, vt_opts)
    ...
    [delta_U, delta_U_mix, v_partial, V_m] = calc_internal_energy_departure(...
        T, P, comp_ref, Pc, Tc, acentric, BIP, M_gmol, vt_type, eos_type, vt_opts);
    ...
    for i = 1:n
        ...
        Q_i_net = (1/tau) * (-delta_U(i) + delta_U_mix * v_partial(i) / V_m);
    ...
    [solution, ~, ~] = fsolve(@(x) residual_firoozabadi(x, params), initial_guess, options);
...
function F = residual_firoozabadi(x, params)
    ...
    if vt_type ~= 0 && vt_type ~= 7
        c_h = calc_vt(z_h_norm, P_h, temp_h, Pc, Tc, acentric, M_gmol, vt_type, eos_type, vt_opts);
...
function [delta_U, delta_U_mix, v_partial, V_m] = calc_internal_energy_departure(T, P, comp, Pc, Tc, acentric, BIP, M_gmol, vt_type, eos_type, vt_opts)
% Internal energy departure for Shukla-Firoozabadi model
%
%   ΔU_i = H_res_i - P*v_i
%
...
%   v_i^VT = v_i^EOS - c_i
    ...
    % Partial molar volumes via analytical formula
    v_partial_eos = calc_partial_molar_volumes(T, P, comp, Pc, Tc, acentric, BIP, eos_type);
    ...
    V_m = sum(comp .* v_partial);

    % Residual enthalpy (VT does NOT affect enthalpy)
    H_res = calc_residual_enthalpy(T, P, comp, Pc, Tc, acentric, BIP, eos_type);

    % Internal energy departure: ΔU_i = H_res_i - P*v_i
    delta_U = zeros(n, 1);
    for i = 1:n
        % delta_U(i) = H_res(i) - P * v_partial(i) + R * T;
        delta_U(i) = H_res(i) ;
    end
...
function v_partial = calc_partial_molar_volumes(T, P, comp, Pc, Tc, acentric, BIP, eos_type)
% Partial molar volumes from EOS
%
%   v_i = RT/P + RT * d(ln φ_i)/dP
    ...
    for i = 1:n
        if phi_P_plus(i) > 0 && phi_P_minus(i) > 0
            ...
            v_partial(i) = R * T / P + R * T * dln_phi_dP;
        else
            v_partial(i) = R * T / P;
        end
    end
...
function H_res = calc_residual_enthalpy(T, P, comp, Pc, Tc, acentric, BIP, eos_type)
% Partial molar residual enthalpies
%
%   H_res_i = -R * T² * d(ln φ_i)/dT
