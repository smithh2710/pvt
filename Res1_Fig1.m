...
for m = 1:4
    ...
    if mid == 3
        [GOC_h, ~, ~, GOC_T] = detect_sgoc(mid, comp_ref, press_ref, temp_ref, h_ref, dTdh, [80 175], Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params, tau);
    else
        [GOC_h, ~, ~, GOC_T] = detect_sgoc(mid, comp_ref, press_ref, temp_ref, h_ref, dTdh, [80 175], Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
    end
    ...
    for i = 1:n_oil
        switch mid
            case 0, [ch,Ph,Th,Pb,~] = main_hasse(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, 0, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 1, [ch,Ph,Th,Pb,~] = main_hasse(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 2, [ch,Ph,Th,Pb,~] = main_kempers(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 3, [ch,Ph,Th,Pb,~] = main_firoozabadi(h_oil(i), h_ref, comp_ref, press_ref, temp_ref, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params);
        end
    ...
    for i = 1:n_gas
        switch mid
            case 0, [ch,Ph,Th,~,Pd] = main_hasse(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), GOC_T, 0, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 1, [ch,Ph,Th,~,Pd] = main_hasse(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), GOC_T, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 2, [ch,Ph,Th,~,Pd] = main_kempers(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), GOC_T, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
            case 3, [ch,Ph,Th,~,Pd] = main_firoozabadi(h_gas(i), GOC_h, comp_gas_ref, oil_P(1), GOC_T, dTdh, Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, tau, vt_method, vt_params);
        end
...
h_leg(5) = plot(NaN, NaN, 'ko', 'MarkerSize', 5, 'MarkerFaceColor', 'k', 'LineWidth', 0.8);
h_leg(6) = plot(NaN, NaN, 'k^', 'MarkerSize', 5, 'MarkerFaceColor', 'w', 'LineWidth', 0.8);
h_leg(7) = plot(NaN, NaN, '-', 'Color', [0.4 0.4 0.4], 'LineWidth', 1.2);
...
leg_str{5} = 'Measured';
leg_str{6} = 'Measured P_{sat}';
leg_str{7} = 'Field GOC (140 m)';

lgd = legend(h_leg(1:7), leg_str, 'Orientation', 'horizontal', ...
    'FontSize', 7, 'NumColumns', 4, 'Box', 'off', 'FontName', 'Helvetica');
lgd.Position = [0.05 0.005 0.92 0.04];

annotation(fig, 'textbox', [0.15 0.96 0.70 0.04], ...
    'String', 'Reservoir 1 — Compositional Grading Comparison (PR-EOS, No VT)', ...
