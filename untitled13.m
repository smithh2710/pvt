...
dTdh = 0.0 ;
[GOC_depth, GOC_pressure, GOC_comp, GOC_temp] = detect_sgoc(1, comp_ref, press_ref, temp_ref, h_ref, dTdh, [80 175], Pc, Tc, acentric, BIP, M_gmol, Cp_coeffs, H_ig_ref, vt_method, vt_params);
% GOC_depth = 136.5938 ; 
...
%% Oil zone
h_oil = [GOC_depth,GOC_depth + 5 , 175, 204, 228, 327];
n_oil = length(h_oil);
...
%% Gas zone
h_gas = [GOC_depth, ( GOC_depth-30 ) , 80, 60, 40, 20, 0];
n_gas = length(h_gas);
...
title('Reservoir 1 — iso Model, No VT');
...
title('Reservoir 1 — isothermal Model, Density vs Depth');
